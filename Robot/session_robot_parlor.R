...
update_profile_vars  <- function(varnames, mins, steps, profile_choice, varname_ref=NULL) {
  lapply (varnames, function(x) {
    observe({
      isolate({ 
        robot_parlor <-input$robot_parlor
      })
      if (robot_parlor=="OFF") { return() }
      if (input$profile_choice== profile_choice) {
        browser()
    i <- which(varnames==paste0(x))
...
update_profile_vars2  <- function(varnames, mins, steps, profile_choice, varname_ref=NULL) {
  lapply (varnames, function(x) {
    observe({
      isolate({ 
        robot_parlor <-input$robot_parlor
        profile <- input$profile_choice
      })
    if ( robot_parlor=="OFF") { return() }
    if ( profile== profile_choice) {
    browser()
    i <- which(varnames==paste0(x))
