...
div(
   fluidRow(column(6, offset=3,div(wellPanel(
             h4(paste(x),align="center")))
             )),
           br(), 
  fluidRow(column(8,offset=2,
           fluidRow(column(6, offset=1, helpText("Weighted average cost of capital after-tax (WACC)")),   
                    column(2, uiOutput(paste0("WACC",refProfile(x))),
                    column(2, helpText("percent"))
           ), 
           a(id = "CF_formula_show_1","Show/hide formula"), 
            shinyjs::hidden(
            div(id = "id_CF_formula_show_1", 
          fluidRow(column(6, helpText("Item")),   
                   column(2, h5("Housing")),
                   column(2, h5("Robot 1")),
                   column(2, h5("Robot 2"))
          ),   
           fluidRow(column(6, helpText("Investment")),   
                    column(2, uiOutput(paste0("CF_invest_housing",refProfile(x)))),
                    column(2, uiOutput(paste0("CF_invest_milking1",refProfile(x)))),
                    column(2, uiOutput(paste0("CF_invest_milking2",refProfile(x))))
           ), 
...
#                     column(2, helpText("percent"))
#            ),
           fluidRow(column(6,  offset=1,helpText("Return on Investment (ROI)")),   
                    column(2, uiOutput("ROI",refProfile(x)))),
                    column(2, helpText("percent"))
           ),
        a(id = "CF_formula_show_2","Show/hide formula"), 
        shinyjs::hidden(
