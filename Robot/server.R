...
shinyServer(function(input, output, session) {
  ...
  # --- Feed --- 
  # output$DMI_change <- 3.507327 # initial value given the initial parameters 
  ...
  IOFC <- reactive({
     (input$milk_cow_day * input$price_milk/100 - rv$DMI_day * input$costDM )*330 - input$additional_labor - input$additional_cost 
  ...
  output$inc_rev_total  <- renderUI({
    rv$inc_rev_total <- rv$inc_rev_herd_size + rv$inc_rev_per_cow + rv$inc_rev_milk_premium +
      ...
      rv$inc_rev_total %>% formatcomma() %>% helpText()
  })
  ...
  output$dec_exp_heat_detection  <- renderUI({
    rv$dec_exp_heat_detection <- (input$hr_heat_detection - input$anticipated_hours_heat )*input$labor_rate *365
      rv$dec_exp_heat_detection %>% formatcomma() %>% helpText()
  })
  ...
  output$dec_exp_labor  <- renderUI({
    rv$dec_exp_labor <- (input$hr_sv_milking + input$hr_heat_detection - input$anticipated_hours_heat )*input$labor_rate *365
      rv$dec_exp_labor %>% formatcomma() %>% helpText()
  })
  
  output$dec_exp_labor_management  <- renderUI({
    rv$dec_exp_labor_management <- input$decrease_lab_mgt * input$labor_rate_rc_mgt * 365
      rv$dec_exp_labor_management %>% formatcomma() %>% helpText()
  })
  
  output$dec_exp_total  <- renderUI({
    rv$dec_exp_total <- rv$dec_exp_heat_detection + rv$dec_exp_labor + rv$dec_exp_labor + rv$dec_exp_labor_management 
      rv$dec_exp_total %>% formatcomma() %>% helpText()
  })
  
  output$positive_total  <- renderUI({
    rv$positive_total <- rv$inc_rev_total +  rv$dec_exp_total
      rv$positive_total %>% formatcomma() %>% helpText()
  })
  
  # --- Negative Impacts ---
  output$inc_exp_repair  <- renderUI({
    rv$inc_exp_repair <-input$repair * input$n_robot + input$insurance_rate * rv$increased_insurance
      rv$inc_exp_repair %>% formatcomma() %>% helpText()
  })
  
  output$inc_exp_feed  <- renderUI({
    rv$inc_exp_feed <-  rv$DMI_change * input$cost_DM * 330 * rv$herd_size2
      rv$inc_exp_feed %>% formatcomma() %>% helpText()
  })
  
  output$inc_exp_pellet  <- renderUI({
    rv$inc_exp_pellet <- input$cost_pellets * 300 * rv$herd_size2 * input$pellets/2000
      rv$inc_exp_pellet %>% formatcomma() %>% helpText()
  })
  
  output$inc_exp_replacement  <- renderUI({
    rv$inc_exp_replacement <- input$cost_heifer * input$change_turnover/100 * rv$herd_size2
      rv$inc_exp_replacement %>% formatcomma() %>% helpText()
  })
  ...
  output$inc_exp_utilities  <- renderUI({
    rv$inc_exp_utilities <- (input$change_electricity + input$change_water + input$change_chemical) * rv$herd_size2
      rv$inc_exp_utilities %>% formatcomma() %>% helpText()
  })
  
  output$inc_exp_record_management  <- renderUI({
    rv$inc_exp_record_management <- input$increase_rc_mgt * input$labor_rate_rc_mgt * 365
      rv$inc_exp_record_management %>% formatcomma() %>% helpText()
  })
  
  output$inc_exp_capital_recovery  <- renderUI({
    browser()
    if (input$n_robot_life > 1) {
      tmp <-  - pmt(input$interest, rv$housing_years, 
                    (rv$total_investment*(1 + input$inflation_robot)^input$robot_years )/(1 + input$interest)^(input$robot_years)) 
    } else {
      tmp <- 0
    }  
     rv$inc_exp_capital_recovery <-   - pmt(input$interest, rv$housing_years, rv$total_investment) + tmp
      rv$inc_exp_capital_recovery %>% formatcomma() %>% helpText()
  })
  ...
  output$inc_exp_total  <- renderUI({
    rv$inc_exp_total <- rv$inc_exp_repair + rv$inc_exp_feed + rv$inc_exp_pellet + rv$inc_exp_replacement + 
      ...
      rv$inc_exp_total %>% formatcomma() %>% helpText()
  })
  
  output$negative_total  <- renderUI({
    rv$negative_total <- rv$inc_exp_total 
      rv$negative_total  %>% formatcomma() %>% helpText()
  })
  
  # --- Net Impacts ---
  output$impact_without_housing  <- renderUI({
    rv$impact_without_housing <- rv$positive_total - rv$negative_total
      rv$impact_without_housing %>% formatcomma() %>% helpText()
  })
  
  output$capital_recovery_housing  <- renderUI({
    rv$capital_recovery_housing <- - pmt(input$interest, rv$housing_years, rv$related_changes)
      rv$capital_recovery_housing %>% formatcomma() %>% helpText()
  })
  
  output$capital_recovery_total  <- renderUI({
    rv$capital_recovery_total <- rv$inc_capital_recovery + rv$capital_recovery_housing
      rv$capital_recovery_total %>% formatcomma() %>% helpText()
  })
  
  output$impact_with_housing  <- renderUI({
    rv$impact_with_housing <- rv$impact_without_housing - rv$capital_recovery_housing 
      rv$impact_with_housing %>% formatcomma() %>% helpText()
  })
  
  output$robot_end_PV  <- renderUI({
    rv$robot_end_PV <- -pmt(input$interest, rv$housing_years, input$salvage_robot/(1 + input$interest)^rv$housing_years)
      rv$robot_end_PV %>% formatcomma() %>% helpText()
  })
  
  output$impact_with_robot_salvage  <- renderUI({
    rv$impact_with_robot_salvage <- rv$impact_with_housing 
      rv$impact_with_robot_salvage %>% formatcomma() %>% helpText()
  })
  
  output$impact_with_inflation  <- renderUI({
    
      rv$impact_with_inflation <- "Depends on cash flow"
      rv$impact_with_inflation %>% formatcomma() %>% helpText()
  })
